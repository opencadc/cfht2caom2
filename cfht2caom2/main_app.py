...
from caom2utils import data_util, fits2caom2
...
# def update(observation, headers, storage_name, file_info, data_client):
...
#     :param headers astropy.io.fits.Headers in a list
#     :param storage_name CFHTName instance
...
#
#     if storage_name.suffix == 'z':
#         ingesting_hdf5 = True
#         logging.info(
#             f'Ingesting the hdf5 plane for {observation.observation_id}'
#         )
#
#     if storage_name.instrument is md.Inst.MEGACAM:
...
#         _update_sitelle_plane(observation, storage_name.file_uri)
#         logging.debug('Done hdf5 update.')
#         return observation
#
#     is_derived, derived_type = _is_derived(
#         headers, storage_name, observation.observation_id
#     )
...
#             if storage_name.has_polarization:
#                 algorithm_name = 'polarization'
#             else:
#                 algorithm_name = 'scan'
#         observation = cc.change_to_composite(observation, algorithm_name)
#
#     if storage_name.instrument is md.Inst.SITELLE and storage_name.suffix == 'v':
#         idx = 0
#     else:
#         idx = _update_observation_metadata(
#             observation, headers, storage_name, data_client
#         )
#     x = instruments.instrument_factory(
#         storage_name.instrument,
#         headers,
#         idx,
#         storage_name,
#         observation,
...
#                 'header': headers[idx],
#             }
#             if get_calibration_level(storage_name) == CalibrationLevel.RAW_STANDARD:
#                 time_delta = get_time_refcoord_delta_simple(
#                     params, storage_name
#                 )
#             else:
...
#                 if (
#                     storage_name.instrument is md.Inst.SPIROU
#                         and storage_name.suffix == 's'
#                 ):
...
#         if (
#             storage_name.instrument is md.Inst.SITELLE
#                 and storage_name.suffix == 'p'
#         ):
#             _update_sitelle_plane(observation, storage_name.file_uri)
#
...
#
#     if (
#         storage_name.suffix in ['p', 'y']
#             and storage_name.instrument is md.Inst.WIRCAM
#     ):
#         # complete the ingestion of the missing bits of a sky construct file
#         _update_wircam_plane(observation, storage_name)
#
#     logging.debug('Done update.')
#     return observation


def get_calibration_level(cfht_name):
...
#
#
def get_exptime(params, cfht_name):
    header, ignore, uri = _decompose_params(params)
...
def get_time_refcoord_delta_simple(params, storage_name):
    # caom2IngestMegacam.py
    exp_time = get_exptime(params, storage_name)
    if exp_time is None:
        header, cfht_name, uri = _decompose_params(params)
        exp_time = mc.to_float(header.get('DARKTIME'))
...
# def _update_observation_metadata(obs, headers, cfht_name, data_client):
#     """
